...
host.exists = function(name) {
  if ( ! name || typeof name !== 'string' ) {
    return log('Please specify a name', 'error');
  }

  log('Search Host by name: ' + name);
...
host.add = function() {
  async.waterfall( 
    ...
    , function (err, results) {
      log('subtree added, back in host.');
    }
  );
}
...
host.update = function() {
  async.waterfall( 
    ...
    , function (err, results) {
      log('subtree updated, back in host.');
    }
  );
}
...
function hasGitRepository(cb) {
  inquirer.prompt({
    ...
    if ( input.hasGit ) {
      ...
      cb(null, {gitUrl: gitUrl});
    }
  });
}

function getHostname(args, cb) {
